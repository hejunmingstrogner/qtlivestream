...
set decs=MFT:d3d=11,hap,D3D11,DXVA,NVDEC,FFmpeg
ver | find "6.0." > nul
if %errorlevel% == 0 goto win7_or_older

set ARCH=%PROCESSOR_ARCHITECTURE%
if [%ARCH%] == [AMD64] set ARCH=x64
:: reg add "HKEY_CLASSES_ROOT\*\shell\Open with MDK D3D11\command" /ve /t REG_SZ /d """"%~dp0bin\glfwplay.exe""" """-logfile""" """%~dp0glfwplay.log"""  """-c:v""" """MFT:d3d=11:pool=1,D3D11,FFmpeg""" """-gl""" """%%1"""" /f
...
:win7_or_older
set decs=hap,D3D11,MFT:d3d=9,NVDEC,FFmpeg
goto :register
...
set glfwplay_args=-plugins mdk-braw -logfile %~dp0glfwplay.log -c:v %decs%,BRAW:scale=%ScreenWidth%x%ScreenHeight% -d3d11
